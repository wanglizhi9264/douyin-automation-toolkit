...
backend 下载器需要在不依赖扩展 UI 的情况下，提供和插件一致的列表扫描、记录恢复、收藏总数、图文/多段下载和失败保护；每个视频段继续使用本仓库原有最高画质实现。

本次范围：

- 下载当前账号喜欢列表中的普通视频、图文和显式多段视频；
- 下载当前账号收藏列表中的同类作品；
...
| BDL-11 | 多媒体续传 | 共享 `mediaParts` 解析；每段成功即保存，重跑跳过已完成 partId，全部完成后才写 downloaded |
| BDL-12 | 收藏概览 | 扫描前读取收藏第一页的远端 total 并复用该页，不重复请求；无 total 时不伪造官方总数 |
...
```json
{
  ...
  "items": [{
    ...
    "mediaCount": 2,
    "mediaParts": [
      { "partId": "image-1", "kind": "image", "order": 1, "url": "https://example.invalid/1.jpg" },
      { "partId": "image-2", "kind": "image", "order": 2, "url": "https://example.invalid/2.jpg" }
    ],
    "downloadedMediaParts": {
      "image-1": { "status": "downloaded", "path": "data/收藏/图片/123/01.jpg" }
    },
    "downloadedPartCount": 1,
    "downloadStatus": "failed",
    "listState": "present",
    "lastError": "second media part failed"
  }]
}
```
...
| BDL-11、BDL-12 | 插件 `test-media-parts.mjs` + backend 路径/候选/收藏 total 契约测试 |
